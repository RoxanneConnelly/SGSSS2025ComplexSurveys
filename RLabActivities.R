...
# Install the packages below (if needed).
  ...
  install.packages(c("tidyverse", "survey", "marginaleffects"))
...
# apply to other models in the Generalized Linear Model family).	
	...
	logit1 <- glm(diabetes ~ weight + age + sex.factor + region.factor,
...
# survey design.
	...
	logit1_adj <- (svyglm(diabetes ~ weight + age + sex.factor + region.factor, 
