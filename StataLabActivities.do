...
/*  You can see that the standard errors of the mean that is only weighted
	are not as large as those seen when the analysis is fully adjusted.	
...
/*	For further guidance on preparing publication ready tables in Stata there
	are additional resources available here 
	(https://www.ncrm.ac.uk/resources/online/all/?id=20788) and 
	here (https://osf.io/6h7gm/), and in the Stata Reporting Reference Manual
...
/* 	As you have seen above, sometimes Stata commands do not work after svy
	...
	(and there is almost often an alternative command that can be used).
	...
	regression model (and the issues described below also apply to other models 
	in the Generalized Linear Model family).	
...
/*	Look carefully at the top portions of both outputs. You can see the 
	...
	model fit statistics cannot be estimated. This will be discussed further 
	in the lecture.
...
/*	You will get an error message above as you cannot calculate AIC and BIC
	...
	There are some very new model fit statistics that have been developed for
	use with logistic regression models but these are not widely used and have
	not yet been implemented in Stata software (although this may be 
	implemented shortly).
	
	The standard practice is to calculate your model fit statistics from
	an unadjusted model as an estimate, and present these alongside your
	adjusted model. 
...
/* 	We do have all the required information to straightforwardly calculate 
	average marginal effects svy: logit, in the same way we would with an 
	unadjusted model.
